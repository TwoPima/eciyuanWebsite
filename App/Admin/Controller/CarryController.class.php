...
class CarryController extends ComController {
	...
	public function index($sid=0,$p=1){
		...
		$list  = $article->field("{$prefix}Carry.*,{$prefix}Carry_category.name")->where($where)->order("{$prefix}Carry.aid desc")->join("{$prefix}Carry_category ON {$prefix}Carry_category.id = {$prefix}Carry.sid")->limit($offset.','.$pagesize)->select();
	...
	public function edit($aid){
		...
		$aid = intval($aid);
		$article = M('Carry')->where('aid='.$aid)->find();
		if($article){
