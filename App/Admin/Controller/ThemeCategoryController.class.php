...
class ThemeCategoryController extends ComController {

	public function index(){
	
		
		$category = M('ThemeCategory')->order('sort asc')->select();
	...
	public function edit(){
		...
		$id = isset($_GET['id'])?intval($_GET['id']):false;
		$currentcategory = M('ThemeCategory')->where('id='.$id)->find();
	...
	public function add(){
		...
		$category = M('ThemeCategory')->field('id,pid,name,link,tag')->order('sort asc')->select();
		$tree = new Tree($category);
		$str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
	...
	public function update($act=null){
		if($act=='order'){
			...
			M('ThemeCategory')->data(array('sort'=>$sort))->where("id='{$id}'")->save();
			addlog('分类修改排序，ID：'.$id);
			die('1');
		}
		...
		if($id){
			if(M('ThemeCategory')->data($data)->where('id='.$id)->save()){
				addlog('文章分类修改，ID：'.$id.'，名称：'.$data['name']);
				die('1');
			}
		}else{
			$id = M('ThemeCategory')->data($data)->add();
			if($id){
