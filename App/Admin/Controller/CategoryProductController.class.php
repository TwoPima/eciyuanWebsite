...
class CategoryProductController extends ComController {

	public function index(){
	
		
		$CategoryProduct = M('CategoryProduct')->field('id,pid,name,o')->order('o asc')->select();
	...
	public function edit(){
		...
		$CategoryProduct = $tree->get_tree(0,$str, $currentCategoryProduct['pid']);
	...
	public function add(){
		...
		$tree = new Tree($CategoryProduct);
		$str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
		$CategoryProduct = $tree->get_tree(0,$str, $pid);
	...
	public function update($act=null){
		if($act=='order'){
			...
			$o = I('post.o',0,'intval');
			M('CategoryProduct')->data(array('o'=>$o))->where("id='{$id}'")->save();
			addlog('分类修改排序，ID：'.$id);
			die('1');
		...
		if($id){
			if(M('CategoryProduct')->data($data)->where('id='.$id)->save()){
				addlog('文章分类修改，ID：'.$id.'，名称：'.$name);
				die('1');
			}
		}else{
			$id = M('CategoryProduct')->data($data)->add();
			if($id){
