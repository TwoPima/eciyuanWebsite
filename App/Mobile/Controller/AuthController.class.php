...
class AuthController extends Controller {
    ...
    private function wechatWebAuth($redirct_url = "",$scope = "snsapi_base"){
    	...
    	$wxurl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".C('APPID')."&redirect_uri=".$redirct_url."&response_type=code&scope=".$scope."&state=STATE#wechat_redirect";
    ...
    private function refreshToken($refresh_token) {
    	...
    	$url = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid=' .C('APPID'). '&grant_type=refresh_token&refresh_token=' . $refresh_token;
    ...
    private function wxGetTokenWithCode($code){
    	...
    	$url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=".C('APPID')."&secret=".C('AESKEY')."&code=".$code."&grant_type=authorization_code";
    	$Token = $this->curlGetInfo($url);
    	$data = json_decode($Token, true);
    	switch ($data['errcode']) {
    		case '40029':
    ...
    protected function get_weixin_access_token() {
    	...
    	$url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".C("APPID")."&secret=".C('AESKEY');
    	$Token = $this->curlGetInfo($url);
    	$data = json_decode($Token, true);
    	if($data['errcode']){
